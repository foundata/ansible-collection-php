...
Main entry point for the foundata.php.run role
...
before installing; package names stay unversioned (`php-*`).

The module stream is managed on EL9 only while a concrete version is pinned
here. In that case `run_php_state: "absent"` also runs `dnf module reset php`,
so an uninstall does not leave the host pinned to this role's PHP version.
Note that the reset is unconditional: if you had enabled a php module stream
yourself before adopting this role, that stream is reset as well and you have
